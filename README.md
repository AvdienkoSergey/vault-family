...
На каждый PR и push в `main` запускаются 7 параллельных job:

```
cargo check                       — компиляция (без swagger)
cargo test                        — тесты
cargo clippy                      — линтер (с -D warnings)
cargo fmt                         — форматирование
cargo check --features swagger    — компиляция с swagger-зависимостями
cargo clippy --features swagger   — линтер swagger-кода (с -D warnings)
